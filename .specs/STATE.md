...
- **Feature**: `self-hosted-docker-bootstrap` (`.specs/features/self-hosted-docker-bootstrap/`)
- **Phase / Task**: Specify → Design → Tasks all approved (22 ACs, 13 tasks T1-T13 across 5 phases). Execute not started - user explicitly said "não vamos executar agora" (2026-08-24).
- **Completed**: none (0/13 tasks)
- **In-progress**: none - nothing mid-write
- **Next step**: When resumed, offer the 2-batch sub-agent split already proposed (Batch1 T1-T6 backend: embedded migrations, embedded SPA, `BootstrapFirst`, `BootstrapHandler`, serve/routes wiring; Batch2 T7-T13 frontend+deploy: `AuthProvider`/`BootstrapPage`/`App.tsx` routing, `Dockerfile`, `docker-compose.yml`, `Makefile`, `README.md`), then dispatch Batch1 first. Verifier runs after T13.
- **Blockers**: none. Flagged risk to remember on resume: T10/T11's Docker gate needs a real `docker` CLI/daemon in whatever environment executes them - if absent, the worker must say so explicitly rather than fake a pass (already written into tasks.md's Test Coverage Matrix caveat).
- **Uncommitted files**: none - `spec.md`/`design.md`/`tasks.md` committed (`0596d08`)
- **Branch**: `main`

**Context for resume**: this feature originated from a user-requested comparative analysis against the open-source project `zeep-orbit` (sibling project, `baas/zeep-orbit`) - specifically to mirror its already-working Dockerfile/docker-compose/bootstrap conventions, not invent new ones. Key facts pulled from reading `zeep-orbit`'s actual source (not memory) that Design relies on: its `Dockerfile` uses `--platform=$BUILDPLATFORM` two-stage builds + `FROM scratch`; its bootstrap race protection uses `LOCK TABLE ... IN EXCLUSIVE MODE` (not `SELECT ... FOR UPDATE`, which locks nothing on an empty table); its embedded-static-file Go source lives inside the same directory tree as the built assets, because `go:embed` cannot use `..` in its pattern - which is why this feature's `web/embed.go` (not an `internal/...` package) mirrors that exact placement. Also discovered mid-Specify: `AD-001`'s "frontend embedded via `go:embed`" was never actually implemented anywhere in this codebase (confirmed via `grep -r "go:embed"` returning nothing before this feature) - this feature is what finally fulfills that decision. A new `AD-009` ("deployment conventions mirror `zeep-orbit`") is planned to be recorded once Execute completes, per design.md's Tech Decisions note - not yet recorded, since the feature hasn't been built yet.
