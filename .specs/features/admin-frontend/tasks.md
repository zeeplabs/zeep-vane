...
> 7. **Fix pós-Verifier (2026-08-21, round 1/3)**: Verifier independente retornou FAIL nesta etapa - 3 gaps reais, não flagados no SPEC_DEVIATION original: (a) AF-28 exigia toast na remoção bem-sucedida de admin, nunca implementado (`sonner` montado em `App.tsx` mas nunca invocado em lugar nenhum do app) - corrigido com `toast.success(...)` em `confirmRemove` (`AdminsPage.tsx`); (b) nenhum teste exercitava o caminho feliz de mudança de papel (AF-27) - mutante que removia `invalidateQueries` de `useUpdateAdminRole.onSuccess` sobrevivia; (c) nenhum teste exercitava o caminho feliz de remoção (AF-28) - mesmo problema em `useDeleteAdmin.onSuccess`; (d) o teste de badge "Pendente" só renderizava a fixture pré-semeada do MSW, nunca o fluxo real de `useInviteAdmin` (AF-25). Adicionados 3 testes novos em `AdminsPage.test.tsx` cobrindo os 3 caminhos felizes (incluindo asserção do texto exato do toast), `<Toaster/>` passou a ser montado no wrapper de teste da página.
