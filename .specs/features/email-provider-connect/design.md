...
| --- | --- | --- | --- |
| **CORRECTION (T5, doc-research verified)**: Resend's docs (`resend.com/docs/api-reference/introduction`, authentication section) state plainly "The API key used was invalid" is a **403**, not 401. The `errors` reference page confirms the same split: `401` covers `missing_api_key` (no `Authorization` header) and one `restricted_api_key` case (valid key, send-only scope hitting a non-send endpoint); `403` covers a genuinely wrong/unrecognized key (`"invalid"`) plus `restricted_api_key`/`suspended_api_key` for an inactive/suspended key. So a submitted garbage key returns 403, not 401 - the opposite of this row's original `[Provável]` assumption. | `internal/connectors/resend/client.go` (new) | None in practice: `ValidateCredentials`'s classifier (mirroring `sendgrid`/`datadog`) already treats **both** 401 and 403 as `email.ErrUnauthorized`, so the wrong-status assumption never mattered functionally. It would have mattered if the connector had classified 401 only. | Implemented `ValidateCredentials`/`Send` to treat 401 and 403 identically, same as the existing SendGrid/Datadog clients - no separate carve-out needed. No further verification required; this row is resolved (`[Certo]`, doc-confirmed, no live key tested). |
