...
| Resend credential-validation call | `GET https://api.resend.com/api-keys` (List API Keys) with the submitted key; 200 = valid, 401 or 403 = invalid/missing key. | [Certo] endpoint and 200 response shape confirmed via `resend.com/docs/api-reference/api-keys/list-api-keys` during Design research (`{object, has_more, data: [{id, name, created_at, last_used_at}]}`). The invalid-key status was re-verified during T5 (implementation): Resend's `errors` reference and `introduction` (authentication) docs state a genuinely wrong/unrecognized key returns **403** ("The API key used was invalid"), not 401 as originally assumed - 401 is reserved for a missing `Authorization` header or a send-only key hitting a non-send endpoint. `ValidateCredentials` classifies both 401 and 403 as invalid (same dual-code pattern the SendGrid/Datadog clients already use), so this correction changes no code, only the documented assumption. Switched from the originally assumed `GET /domains`: listing API keys has no side effects and does not depend on the owner having verified a sending domain yet (a fresh Resend account may have zero domains and would otherwise look invalid even with a working key). | y (research-confirmed, T5) |
