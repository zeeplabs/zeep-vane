...
3. WHILE a replica holds the lock, it SHALL renew it (or otherwise assert liveness) on every successful poll cycle, at least once per `PollerManager` renewal interval.
