...
| Recovery boundary that allows proposing incident closure | Any `→ operational` transition checks for a still-open **auto-created** incident linked to that service (`outage` → `degraded` leaves the incident open, since it never reaches `operational`); closure is only proposed once every service linked to that incident is also `operational` | Confirmed with Julio — `degraded` is still a real problem, closing on a partial recovery would misrepresent status; corrected post-review (2026-09-09) to also cover multi-service incidents (`incident_services` is N:N) and to exclude manually-created incidents, which must never receive an LLM-drafted closing proposal | y |
...
| Idempotency of auto-created incidents | At most one open (non-resolved) **auto-created** incident per service at a time; a service that flaps outage→degraded→outage while already having an open auto-created incident does not create a second one. Scoped to `auto_created = true` so a manually-created incident (e.g. a maintenance window) never suppresses a real outage incident for the same service | Prevents duplicate/overlapping public incidents for the same ongoing problem, without letting an unrelated manual incident mask a real one | n — agent's discretion, logical consequence of "one incident per service transition"; auto_created scoping added post-review (2026-09-09) |
...
5. IF a service already has an open (non-resolved) **auto-created** incident THEN a repeated `outage` transition for that same service SHALL NOT create a second incident. A manually-created incident (not auto-created) linked to the service SHALL NOT suppress creation of a real outage incident.
