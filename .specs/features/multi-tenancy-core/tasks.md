...
**Fix round (2026-09-10) - wiring + RLS gap closed, see AD-024:** both deferrals above are now resolved. (1) `internal/cli/serve.go`'s `newPollerFromStoredIntegration` calls `EnableTenantIteration(db.NewSystemTenantLister(pool), poolTenantTx(pool))`, so the production poller really does iterate every active tenant with `app.tenant_id` set per tenant; `poolTenantTx` is a thin adapter over `Pool.BeginTenantTx` (`app.user_id` left unset - the poller acts as no user). (2) The bootstrap paradox this task flagged is closed by migration `0027_poller_tenant_enumeration`: a PERMISSIVE `FOR SELECT` policy on `tenants` gated on `current_setting('app.is_system', true) = 'true'`. That flag is set in exactly one place, `db.SystemTenantLister.List`, on its own transaction, with a fixed literal and no user input anywhere in the call path, covering only the enumeration query; every per-tenant poll afterwards runs on the ordinary `app.tenant_id` session. AD-023's condition was deliberately *not* reused - `tenants` has no "published" concept, and gating on "no active tenant" would expose the tenant list to the anonymous public-status-page path. New tests: `internal/db/system_tenant_lister_test.go` (3, enumeration under a real non-superuser login role + fail-closed control + flag does not outlive its transaction), `internal/db/system_flag_scope_test.go` (1, structural - no production file outside the owner mentions `app.is_system` in executable code), `internal/api/system_flag_http_test.go` (1, authenticated request never carries the flag), `internal/router/system_flag_anonymous_test.go` (1, anonymous public-status request never carries it), `internal/cli/poller_tenant_wiring_test.go` (2, production construction path enables tenant iteration; the adapter really scopes the session). `extraPolicies` in `internal/db/multi_tenancy_migration_test.go` gained the `tenants` entry.
