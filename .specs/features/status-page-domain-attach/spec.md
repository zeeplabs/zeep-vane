...
| Concurrent attach requests racing on the same status page | Handled with `SELECT ... FOR UPDATE` on the target row inside an explicit transaction before deciding the outcome; the loser's SELECT blocks until the winner commits, then observes the now-non-null `domain_id` and gets `409`, exactly like the sequential case | Locking the row lets a single query resolve which of the 4 distinguishable outcomes applies before acting - a conditional `UPDATE` affecting 0 rows can't tell "page doesn't exist" apart from "page already has a domain" (design.md Tech Decisions). | y |
