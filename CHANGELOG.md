...
- `VANE_ADMIN_BASE_URL` validation only checked for a `http://`/`https://` prefix, so a bare `https://` (no host) or a value that trims to empty (e.g. `/`) still passed and would have produced a broken email link or been silently treated as unconfigured. Now parsed as a URL and requires both a scheme and a non-empty host.
