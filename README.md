...
| `VANE_NOTIFICATION_SERVICE_BASE_URL` | Only when `VANE_DEPLOYMENT_MODE=saas` | *(empty)* | Base URL of `zeep-notification-service`, the single channel every transactional email (signup verification, password reset, admin invite, incident opened/resolved, weekly digest) goes through in `saas` mode (AD-034) — a saas tenant never connects its own email provider (`/api/integrations/email/*` returns `404` there instead). Unused, and safe to leave unset, in `self_hosted` mode. **`vane serve` refuses to start in `saas` mode without this set** |
| `VANE_NOTIFICATION_SERVICE_API_KEY` | Only when `VANE_DEPLOYMENT_MODE=saas` | *(empty)* | Single platform-wide credential authenticating every call to `zeep-notification-service` above — never a per-tenant key. **`vane serve` refuses to start in `saas` mode without this set** |
