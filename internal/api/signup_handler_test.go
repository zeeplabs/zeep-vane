...
func newSignupRouterWithEmail(t *testing.T, emailSvc *email.Service) (http.Handler, *db.Pool, *db.UserRepository) {
	...
	authHandler := NewAuthHandler(users, memberships, db.NewTwoFactorRepository(pool), db.NewTwoFactorChallengeRepository(pool), db.NewSessionRepository(pool), pool, zap.NewNop(), testSessionSecret, true, testMasterKey)
