...
func newTenantMembershipRepoTestPool(t *testing.T) (*TenantMembershipRepository, *TenantRepository, *AdminRepository, *Pool) {
	t.Helper()
	...
	return NewTenantMembershipRepository(pool), NewTenantRepository(pool), NewAdminRepository(pool), pool
}

// createMembershipTestAdmin inserts a plain admin row (identity only -
// tenant_memberships.user_id FKs to admins(id) in this batch, see T1's
// SPEC_DEVIATION note), registering its cleanup.
func createMembershipTestAdmin(t *testing.T, admins *AdminRepository, pool *Pool, email string) *Admin {
	t.Helper()
	...
	t.Cleanup(func() { _, _ = pool.Exec(context.Background(), "DELETE FROM admins WHERE id = $1", admin.ID) })
	return admin
}
